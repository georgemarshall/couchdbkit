...
from .exceptions import (
    InvalidAttachment, ResourceNotFound, ResourceConflict, BulkSaveError,
...
class Database(object):
    ...
    def __init__(self, server, name, **params):
        ...
        """
        # Only place we need `Server`, import here to prevent a circular import.
        ...
        # Quick check to make sure the database exsist
        self.resource.head()
    ...
    def doc_exist(self, docid):
        ...
        :rtype: `boolean`, True if document exist
        """

        try:
            self.resource.head(resource.escape_docid(docid))
    ...
    def get_doc(self, _id):
        ...
        @return: dict, representation of CouchDB document as
         a dict.
        """
        # TODO: We need to lookup the doc_type and wrap it
        data = self.resource.get(urllib.quote_plus(_id)).json_body
        kwargs = dict((str(key), value) for key, value in data.iteritems())
        return self.document_class(self, **kwargs)
        # wrapper = Document
        ...
        # doc = self.resource.get(docid, **params).json_body
    ...
    def new_doc(self, _id=None, **data):
        """
        ...
        :param doc: A :dict: doc is updated with doc '_id' and '_rev' properties returned by CouchDB server when you save.
        :param force_update: boolean, if there is conlict, try to update with latest revision
        :param params: list of optionnal params, like batch="ok"
        :rtype: :class:`Document <couchdbkit.schema.Document>`
        """
        return self.document_class(self, _id, **data)
        # if doc is None:
        #     doc1 = {}
        # else:
...
class TempView(ViewInterface):
    ...
    def _exec(self, **params):
        return self._db.res.post('_temp_view', payload=self.design, **params)
